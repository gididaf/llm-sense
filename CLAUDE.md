...
3. **Task Generation** — Claude generates difficulty-tiered synthetic bugs + features using Phase 1 structural data (hub files, complexity hotspots, module boundaries)
4. **Empirical Testing** — Tasks run in parallel across isolated git worktrees, with correctness verification
5. **Scoring** — Weighted aggregation across 12 categories → 0-100 score (with difficulty-weighted empirical scoring and custom profile support)
...
3. Same-category recommendations are automatically bundled into a single Claude session (e.g., all File Sizes recs — code splits, data extraction, vendored exclusion — run together for maximum impact)
4. For each bundle: create worktree → sync uncommitted changes from working dir → score baseline (overall + category) → build prompt from recommendation → call Claude Code → validate build → re-score → merge if improved
5. Build validation: detects package manager (pnpm/yarn/npm) and build commands from workspace package.json files. For monorepos with no root build script, scans subdirectories with changed files for build commands. Changes that break compilation are rejected. The fix prompt also instructs Claude to run the build itself and fix errors.
6. Merge decision: accepts changes if overall score improved OR if the targeted category score improved and overall didn't decrease. This prevents good single-category work from being discarded due to unrelated categories anchoring the overall score.
7. Split tasks get 30min timeout and 50 max turns. Bundled tasks get $10 budget. Other tasks get 10min timeout, 30 turns, $5 budget.
...
- **Difficulty-tiered empirical testing:** Task generation receives Phase 1 structural data (hub files, complexity hotspots, orphan files, cross-directory coupling, god files) and enforces difficulty quotas: 40% easy, 40% medium, 20% hard (from `DIFFICULTY_DISTRIBUTION` in constants). Easy tasks target isolated single-file changes; medium tasks span 2-3 files within a module; hard tasks cross module boundaries and touch hub files. If Claude deviates from the quota, difficulty labels are reassigned based on `expectedFilesTouch.length`. Scoring uses `DIFFICULTY_WEIGHTS` (easy=0.5x, medium=1.0x, hard=2.0x) for weighted success rate and correctness. `TaskExecutionResult` includes `taskDifficulty` field. JSON output includes per-difficulty breakdown.
...
- **DevInfra scoring prioritizes LLM-relevance:** The devInfra analyzer checks 8 areas but weights them by how much they help LLMs: tests (4pts), linter (3pts), type checking (3pts), pre-commit hooks (2pts), observability (2pts) are high-value because they give LLM agents feedback loops. CI (1pt), devcontainer (1pt), task discovery (1pt) are bonus points — nice project hygiene but don't directly help LLMs. Total max: 17 raw, scaled to 0-100 via `score * (100/17)`. Recommendations only suggest LLM-relevant items (tests, linter, types, hooks), never CI/devcontainer/templates/changelog.
- **Linter detection checks subdirectories:** The devInfra linter detection scans the root path AND immediate subdirectories (for monorepo workspace packages). This ensures `backend/.eslintrc.json` is found even without a root-level linter config. The walkDir-based entries don't include dotfiles, so explicit path checks are needed.
- **Documentation scoring: README not required when CLAUDE.md exists:** If a comprehensive CLAUDE.md exists (4+ of 8 sections), the README.md points (10pts) are awarded automatically — README is redundant for LLMs when CLAUDE.md provides equivalent orientation. The recommendation to add README.md is also suppressed.
...
- **Auto-fix build validation:** Changes that break compilation are automatically rejected. Package manager is auto-detected (pnpm/yarn/npm) via lockfile presence. For monorepos, build commands are discovered from workspace package.json files in directories with changed files. The fix prompt also instructs Claude to verify the build passes and fix errors proactively.
- **Auto-fix category-aware scoring:** The merge decision checks both overall score AND the targeted category score (from `rec.category`). If the category improved but overall is flat (due to unrelated large files etc.), changes are still accepted as long as overall didn't decrease. This prevents $10 of good file-split work from being discarded because one unrelated god file anchors the overall score.
- **Auto-fix same-category bundling:** When selecting recommendations to fix, all same-category recs are bundled into a single Claude session. For `--fix-count 3`, if the top 3 recs span 2 categories, they become 2 bundled tasks. For `--fix-id`, the targeted rec plus all same-category siblings are bundled. This ensures maximum impact per session (e.g., splitting code files + extracting data files + excluding vendored files all happen together).
- **Batched file-split recommendations cap at 3 files:** Splitting more than 3 files in a single Claude Code session tends to time out. The batch uses $10 budget and 1800s timeout vs $5/600s for single-file tasks. Individual per-file recommendations are still generated alongside the batch for `--fix-id` targeting.
- **Auto-fix worktree sync:** When running multiple auto-fix passes, each new worktree syncs uncommitted/untracked changes from the working directory. This ensures subsequent fixes build on improvements from previous passes rather than starting from git HEAD each time.
- **Auto-fix file-split tuning:** Split tasks get 1800s timeout (30min) and 50 max turns (vs 600s/30 turns for non-split tasks). The prompt includes efficiency guidelines: modify the original file first (extract-then-remove per chunk), limit to 3-5 output files, keep original as barrel re-export, no codebase-wide import scanning.
- **Auto-fix graceful exit code 1:** When Claude Code exits with code 1 (max turns/budget exceeded), the output is parsed for cost/turn data. If valid, the worktree changes are still evaluated for score improvement rather than being thrown away. Partial completions that improve the score are accepted.
